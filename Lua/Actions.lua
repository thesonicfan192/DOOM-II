...
local function P_TryMove(thing, x, y)
    -- mirror locals from C
    local oldx, oldy
    local side, oldside, ld
	local newSector = R_PointInSubsector(x, y).sector
	local tmfloorz = thing.subsector.sector.floorheight
	local tmceilingz = thing.subsector.sector.ceilingheight
	local tmdropoffz = newSector.floorheight

    -- Start: check whether position is free (P_CheckPosition sets tmfloorz/tmceilingz etc)
    local floatok = false
    if not P_CheckPosition(thing, x, y) then
        return false
    end

    if not (thing.flags & MF_NOCLIP) then
        -- won't fit between floor/ceiling?
        if (tmceilingz - tmfloorz) < thing.height then
			print("Can't fit in the new sector!")
            return false
        end

        floatok = true

        if not (thing.flags & DF_TELEPORT) and (tmceilingz - thing.z) < thing.height then
			print("Can't fit in the new sector's ceiling!")
            return false
        end

        if not (thing.flags & DF_TELEPORT) and (tmfloorz - thing.z) > (24 * FRACUNIT) then
			print("Can't fit in the new sector's floor!")
            return false
        end

        if not (thing.flags & (DF_DROPOFF | MF_FLOAT)) and (tmfloorz - tmdropoffz) > (24 * FRACUNIT) then
			print("Drop-off is too large!")
            return false
        end
    end

	local result P_MoveOrigin(thing, x, y, thing.z)
	print("Move result was " .. tostring(result))

    -- handle special lines if any (only if not teleporting/no-clipping)
    if not (thing.flags & (DF_TELEPORT | MF_NOCLIP)) then
        -- emulate C's while(numspechit--) { ld = spechit[numspechit]; ... }
        while (numspechit and numspechit > 0) do
            numspechit = numspechit - 1

            -- try to read spechit in a C-style (0-based) or Lua-style (1-based) table
            ld = spechit[numspechit] or spechit[numspechit + 1]

            if not ld then
                -- nothing at this slot; continue defensively
            else
                side = P_PointOnLineSide(thing.x, thing.y, ld)
                oldside = P_PointOnLineSide(oldx, oldy, ld)
                if side ~= oldside then
                    -- only call cross if the line has a special
                    if ld.special and ld.special ~= 0 then
                        -- P_CrossSpecialLine in C expects an index (ld - lines).
                        -- Try to provide a numeric index if available; otherwise pass 'ld' and hope binding accepts it.
                        local ldidx = line_id_for_call(ld)
                        if P_CrossSpecialLine then
                            -- try to call with (index, oldside, thing)
                            -- some hosts expect (lineIndex, side, thing)
                            local ok, err = pcall(function() P_CrossSpecialLine(ldidx, oldside, thing) end)
                            if not ok then
                                -- fallback: try passing the raw ld
                                pcall(function() P_CrossSpecialLine(ld, oldside, thing) end)
                            end
                        end
                    end
                end
...
local function DOOM_Move(actor, speed)
    local tryx, tryy
    local ld
    ...
    -- sanity check (actor.movedir expected in 0..7)
    if actor.movedir < 0 or actor.movedir >= #xspeed then
        error("Weird actor->movedir!")
    end

    -- C code used direct fixed multiply; keep the same simple multiply
    local idx = actor.movedir + 1 -- convert 0-based movedir to Lua 1-based table index
    tryx = actor.x + (speed * xspeed[idx])
    tryy = actor.y + (speed * yspeed[idx])

    local try_ok = P_TryMove(actor, tryx, tryy)
	local tmfloorz = actor.subsector.sector.floorheight

    if not try_ok then
        -- floating actor may adjust height and succeed
        if (actor.flags & MF_FLOAT) ~= 0 and floatok then
            if actor.z < tmfloorz then
                actor.z = actor.z + FLOATSPEED
            else
                actor.z = actor.z - FLOATSPEED
            end
            actor.flags = actor.flags | DF_INFLOAT
            return true
        end

        -- if no specials were hit, move blocked
        if not numspechit or numspechit == 0 then
            return false
        end

        -- process specials that were hit; emulate C's while (numspechit--) { ... }
        ...
        while (numspechit and numspechit > 0) do
            ...
            ld = spechit[numspechit] or spechit[numspechit + 1]
            if ld then
                -- Try calling P_UseSpecialLine(actor, ld, 0)
                local ok, res = pcall(function() return P_UseSpecialLine(actor, line_id_for_call(ld), 0) end)
                if not ok then
                    -- fallback: try passing raw ld
                    ok, res = pcall(function() return P_UseSpecialLine(actor, ld, 0) end)
                end
                if ok and res then
                    good = true
                end
    ...
    if not (actor.flags & MF_FLOAT) then
...
function A_DoomChase(actor)
	...
	// Chase toward player
	actor.movecount = $ - 1
	if actor.movecount < 0 or not DOOM_Move(actor, actor.info.speed / FRACUNIT) then
		P_NewChaseDir(actor)
	end
